    ...
    function sendOrderEmails(array $orderData, array $emails, string $path_url)
    {
        ...
        try {
            ...
            $mail->CharSet = 'UTF-8'; // quan trọng: mã hóa UTF-8
            $mail->isSMTP();
     ...
     *
     * @param array $contactData  Mảng thông tin liên hệ: name, phone, email, address, message
     * @param array $emails       Mảng email nhận ['admin' => '...', 'customer' => '...']
     */
    function sendContactEmail(array $contactData, array $emails)
    {
        ...
        try {
            ...
            $mail->setFrom(MAIL_FROM, MAIL_FROMNAME);
            $mail->isHTML(true);
            $mail->Subject = "Khách hàng liên hệ từ website";
        ...
        ";
            ...
            if (!empty($emails['admin'])) {
                $mail->clearAllRecipients();
                $mail->addAddress($emails['admin'], 'Admin');
                $mail->send();
            }

            // Gửi cho khách hàng (nếu muốn)
            if (!empty($emails['customer'])) {
                $mail->clearAllRecipients();
                $mail->addAddress($emails['customer'], $name);
                $mail->send();
            }
